...
module.exports = function (app) {
	...
	(req, res) => 
	{
		...
		if (expression)
			return res.status(201).send(data); return res.status(500);
	})

	/* NOTE: Completing informations automaticaly obtaineds */
	app.get('/automatic_and_incremented/user/:id', 
	(req, res) => {
		/* 	#swagger.tags = ['User'];
		...
		if (expression) {
			/* #swagger.responses[200] = { schema: { "$ref": "#/definitions/User" }, description: "User registered successfully." } */
...
function myFunction2(p) {
	...
	//	#swagger.method='put';#swagger.description='Forced endpoint.'; 	#swagger.produces    =      ["application/json"]
